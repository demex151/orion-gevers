...
class SearchFinding:
    ...
    def __post_init__(self):
        ...
        self.domain = (self.domain or "").strip().lower() or (
            (urlparse(self.url).hostname or "").lower() if self.url else ""
        )
...
class DdgsSearchProvider:
    ...
    def __init__(self, max_results=6, region="us-en", backend="auto", ddgs_factory=None):
...
class GeversLeadProfile:
    ...
    def queries(self):
        locations = ("Myrtle Beach", "Horry County")
        intents = (
            "looking for painter",
            "need painter",
            "painter recommendations",
            "need painting estimate",
        )
        return [f'"{intent}" "{location}"' for location in locations for intent in intents]
