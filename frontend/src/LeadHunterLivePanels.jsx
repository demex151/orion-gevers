...
import {currentGeverRuntimeState,nextPanelVisibility} from './leadPanelSpeechLifecycle.js';
...
export default function LeadHunterLivePanels(){
 const [progress,setProgress]=useState(null),[results,setResults]=useState(null),[panelState,setPanelState]=useState({visible:false,sawSpeaking:false});
 const lastState=useRef('idle'),lastDisplayRequest=useRef(0);
 const loadResults=async()=>{try{const rr=await fetch('http://127.0.0.1:8000/api/lead-hunter/results'),rj=await rr.json();if(rj.ok){setResults(rj);setPanelState({visible:true,sawSpeaking:false})}}catch{}};
 useEffect(()=>{let alive=true,timer;const poll=async()=>{try{const r=await fetch('http://127.0.0.1:8000/api/lead-hunter/progress'),j=await r.json();if(alive&&j.ok){const p=j.progress;setProgress(p);if(p?.state==='completed'&&lastState.current!=='completed')await loadResults();const request=Number(p?.display_request||0);if(request>lastDisplayRequest.current){lastDisplayRequest.current=request;await loadResults()}else lastDisplayRequest.current=Math.max(lastDisplayRequest.current,request);lastState.current=p?.state||'idle'}}catch{}if(alive)timer=setTimeout(poll,350)};poll();return()=>{alive=false;clearTimeout(timer)}},[]);
 useEffect(()=>{const timer=setInterval(()=>setPanelState(state=>state.visible?nextPanelVisibility(state,currentGeverRuntimeState()):state),100);return()=>clearInterval(timer)},[]);
 if(panelState.visible&&results)return <StoredResults payload={results}/>;
 const panels=telemetryPanels(progress);if(!panels.length||progress?.state==='completed')return null;
